...
def test_intents_on_public_exports_resolve() -> None:
    pkg = importlib.import_module("aoa.action_machine.intents.on")
...
def test_domain_public_exports_resolve() -> None:
    domain = importlib.import_module("aoa.action_machine.domain")
...
def test_intents_entity_exports_resolve() -> None:
    ent = importlib.import_module("aoa.action_machine.intents.entity")
...
def test_intents_meta_lazy_meta_decorator_resolve() -> None:
    meta_pkg = importlib.import_module("aoa.action_machine.intents.meta")
...
def test_auth_lazy_pipeline_symbols_resolve() -> None:
    auth = importlib.import_module("aoa.action_machine.auth")
    for name in (
        "Authenticator",
        "AuthCoordinator",
        "ContextAssembler",
...
def test_context_requires_exports_intent_surface() -> None:
    ctx_pkg = importlib.import_module("aoa.action_machine.intents.context_requires")
...
def test_intents_role_mode_lazy_decorator_symbols_resolve() -> None:
    rm = importlib.import_module("aoa.action_machine.intents.role_mode")
    assert rm.RoleMode is not None
    assert rm.role_mode is not None


def test_intents_connection_lazy_symbols_resolve() -> None:
    conn = importlib.import_module("aoa.action_machine.intents.connection")
