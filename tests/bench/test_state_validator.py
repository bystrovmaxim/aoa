...
def _coord_aspects(c: GateCoordinator, cls: type):
    snap = c.get_snapshot(cls, "aspect")
    return getattr(snap, "aspects", ()) if snap is not None else ()


def _coord_checkers_for_aspect(c: GateCoordinator, cls: type):
...
class TestValidateForAspect:
    ...
    def test_single_aspect_action(self, coordinator: GateCoordinator) -> None:
        ...
        validate_state_for_aspect(aspects, chk, "validate_name_aspect", {})
...
class TestErrorAttributes:
    ...
    def test_nonexistent_aspect_has_no_field(self, coordinator: GateCoordinator) -> None:
        ...
        with pytest.raises(StateValidationError) as exc_info:
            validate_state_for_aspect(aspects, chk, "no_such_aspect", {})
