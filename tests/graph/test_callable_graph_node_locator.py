...
"""Callable graph node locator behavior."""

from action_machine.model.graph_model.action_graph_node import ActionGraphNode
from action_machine.model.graph_model.callable_graph_node_locator import (
    CallableGraphNodeLocator,
)
...
def test_get_summary_aspect_edges_builds_edges() -> None:
    ...
    assert CallableGraphNodeLocator.get_summary_aspect_edges(node, PingAction) == node.summary_aspect_edges


def test_get_regular_aspect_edges_builds_edges_with_target_nodes() -> None:
    node = ActionGraphNode(ChildAction)

    edges = CallableGraphNodeLocator.get_regular_aspect_edges(node, ChildAction)
...
def test_get_compensator_edges_builds_edges() -> None:
    ...
    assert CallableGraphNodeLocator.get_compensator_edges(node, ChildAction) == node.compensator_graph_edges


def test_get_error_handler_edges_builds_edges() -> None:
    node = ActionGraphNode(ChildAction)

    assert CallableGraphNodeLocator.get_error_handler_edges(node, ChildAction) == node.error_handler_graph_edges
