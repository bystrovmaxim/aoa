### Обновлённые файлы PostgreSQL-сохранятелей без удаления старых записей

#### 1. `YouTrackMCP/YouTrackStoriyIssuesPostgresSaver.py`
...
#### 2. `YouTrackMCP/YouTrackTasksIssuesPostgresSaver.py`
...
- Удалён блок `DELETE` и связанная с ним переменная `deleted`.
- Убран декоратор `@IntFieldChecker("deleted", ...)`.
- Возвращаемый словарь теперь содержит только `inserted` (количество обработанных строк).
- Комментарии обновлены, чтобы отражать новое поведение (upsert без предварительной очистки).

Такая логика позволяет:
- При первом запуске за дату – вставить все записи.
- При повторном запуске за ту же дату – обновить существующие записи (если данные изменились) и добавить новые, если появились задачи.
- Старые задачи, которые больше не возвращаются YouTrack, останутся в базе (они не удаляются). Это может быть как плюсом (история сохраняется), так и минусом (если нужно точное состояние на момент снимка). В вашем случае, вероятно, это приемлемо. Если требуется полная замена снимка, можно вернуть `DELETE`, но по запросу мы его убрали.