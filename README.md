...
- **Service layer** — the outer boundary of the system, where transports and public interfaces are chosen. Its elements are adapters (`FastApiAdapter`, `McpAdapter`, `CLIAdapter`, `GraphQLAdapter`, …): they publish the same `Action` operations over HTTP, MCP, CLI, or GraphQL without duplicating business logic.
