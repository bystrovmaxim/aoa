...
- **`CacheKeyMixin` — hash-based cache key and unconditional write policy.** A new mixin class in `aoa.action_machine.model`. Place it before `BaseAction` in the MRO to enable full result caching for an action: `cache_key` returns a stable SHA-256 hex digest of all scalar fields (`str`, `int`, `float`, `bool`, `None`) in `params`; `on_cache_write` always returns `True`. Non-scalar fields are excluded from the key. ([#56](https://github.com/bystrovmaxim/aoa/issues/56))
