...
- **`aoa-otel` package: OpenTelemetry plugin as a standalone distribution.** A new separate package `aoa-otel` (`pip install aoa-otel`) provides `OpenTelemetryPlugin` under the `aoa.otel` namespace. The package depends on `aoa-action-machine`, `opentelemetry-api`, and `opentelemetry-sdk`. ([#70](https://github.com/bystrovmaxim/aoa/issues/70))
...
- **`aoa-mcp-adapter` package: `McpAdapter` as a standalone distribution.** A new separate package `aoa-mcp-adapter` (`pip install aoa-mcp-adapter`) provides `McpAdapter` and `McpRouteRecord` under the `aoa.mcp` namespace. The package depends on `aoa-action-machine` and `mcp`. ([#63](https://github.com/bystrovmaxim/aoa/issues/63))
- **`aoa-ocel` package: OCEL 2.0 export plugin as a standalone distribution.** A new separate package `aoa-ocel` (`pip install aoa-ocel`) provides `OcelPlugin`, `OcelFrame`, `InMemoryOcelStoreResource`, `OcelStoreResource`, `OCEL_FRAMES_KEY`, and supporting DTOs under the `aoa.ocel` namespace. The package depends on `aoa-action-machine` and `xxhash`. ([#71](https://github.com/bystrovmaxim/aoa/issues/71))
