...
- **Graph module layout and names.** Removed `action_machine.graph.logical` (types and builders now live directly under `action_machine.graph`). `LogicalVertex` / `LogicalEdge` → `GraphVertex` / `GraphEdge`; `LogicalGraphBuilder` → `GraphBuilder`; `build_from_g0_input` → `build_from_synthetic_bundle`; `g0_builder` module → `synthetic_input_builder`; `logical_dag` → `dag`; facet→interchange lives in `action_machine.graph.interchange_from_facets` (no separate `facet_graph` module; no payload-type filtering — stable sort by `(node_type, node_name)` only). Golden fixtures: `logical_minimal.json` → `synthetic_minimal.json`, `logical_test_domain.graph.json` → `test_domain.graph.json`. Tests moved from `tests/graph_logical_contract/` to `tests/graph_contract/`.
...
- **Resource manager modules.** `resource_managers.connection` → `connection_decorator`; `resource_managers.connections` → `connections_typed_dict`. Canonical package: `action_machine.resources`. The old `resource_managers` package directory was removed; imports use `action_machine.resources` only.
...
- **Graph package.** `action_machine.metadata` → `action_machine.graph` (`GraphCoordinator`, `FacetPayload`, `BaseIntentInspector`, facet snapshots, `graph_execution_adapters`, exceptions, cleanup). All `*IntentInspector` modules live under `action_machine.graph.inspectors.*`; imports from `action_machine.metadata` are removed.
...
- `**extract_action_params_result_types`.** Single location: `action_machine.runtime.binding.action_generic_params`.
