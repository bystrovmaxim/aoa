<!-- translated-from: glossary_draft.md @ 2026-07-19T17:47:30Z (filesystem mtime; draft is gitignored, no git history) · sha256:a28dc91821a7 -->
<p align="center">
...
**`BaseVerdict`** — the abstract root of the outcome of any access check (`aoa.action_machine.intents.access_control`); it cannot be instantiated directly (the constructor raises `TypeError`). `kind` isn't a field of its own but a computed one (`@computed_field`) — the class's own name (`type(self).__name__`), defined once on the base class and inherited without redeclaration. Three concrete descendants:
