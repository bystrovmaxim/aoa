...
**`reason`** — the only substantive field on `FailSecurityVerdict`/`FailErrorVerdict` (on `AllowedVerdict` there's no such field at all — not an empty string): a non-empty, declared-ahead-of-time string. The same principle applies one layer down, as the mandatory companion of `when=`/`guard=`: `grant(role, when=..., reason=...)`/`check_roles(guard=..., reason=...)` take a `FailSecurityVerdict`, not a bare string. If `when=`/`guard=` is given without `reason=`, a default is substituted — `FailSecurityVerdict("FORBIDDEN_GRANT")`/`FailSecurityVerdict("FORBIDDEN_GUARD")` — a deliberate relaxation of the earlier rule (giving `when=`/`guard=` without `reason=` used to be a `ValueError`); the reverse case (`reason=` without `when=`/`guard=`) is still a `ValueError`. A role that never matched at all (no grant won) is the fixed `"FORBIDDEN_ROLE"`, not something the developer declares. One more fixed source the developer never declares — `"UNAUTHORIZED"` (`aoa-fastapi-adapter`): the route's own auth gate rejected the caller before the role/`guard`/`access_decide` cascade even started — a settled denial, not a check that was never reached, just with the route itself as the source instead of the action. `access_decide` (level 3) is no longer the exception: it returns `FailSecurityVerdict(reason=...)` with its own declared reason, same as the other levels. Two further fixed sources the developer never declares: `"FORBIDDEN_OBJECT"` (`access_decide` — the generic object-level denial, see below) and `"EVALUATION_FAILED"` (not a denial but a `FailErrorVerdict` — the check itself crashed, see **oracle safety**).

**Role-gate** and **Object verdict** — a role-based check that ignores the specific object (RBAC, cascade levels 1-2: role and `guard=`), and a check based on a fact about a specific object (ABAC, level 3, `access_decide`), respectively. Indistinguishable by `kind` on the wire — both give the same `kind: "FailSecurityVerdict"`, and the only thing that can tell them apart is the text of `reason`, if the developer chose to reveal it. Within an object verdict, two specific cases — "no such object" and "object belongs to someone else" — are required to answer with the same `FailSecurityVerdict("FORBIDDEN_OBJECT")` (`aoa.action_machine.intents.access_control`, a ready-made reusable value, not text written from scratch in each action) — precisely so as not to be an oracle for the existence of other people's objects. This requirement is about that one pair, not a general rule for every `reason`: other reasons (a role-gate's `"FORBIDDEN_ROLE"`, say) keep their own distinct text, because they give nothing away about anyone's specific objects.

**Oracle safety** — the principle that an answer to a permission question must not let an outsider tell "this object doesn't exist" from "this object exists but isn't yours" by probing identifiers. A server that answers those two differently acts as an **oracle** — it inadvertently reveals which IDs exist for other users. Hence the second rule too: a crash in the check itself (the database fell over, a bug) is not a denial but a separate third answer, `FailErrorVerdict("EVALUATION_FAILED")`; treating a crash as a denial makes "a real denial" and "a crash" distinguishable by the response again — the same hole from the other side. A specific denial reason (e.g. "this order is archived") only stays safe once ownership has already been confirmed — before that point, any reason more precise than `FORBIDDEN_OBJECT` can serve as an oracle.
...
**Contract fixture** — one JSON file in `contracts/fixtures/` representing a whole `POST /permissions/resolve` response body, read by **both** halves of the system: by the Python `ResolveResponse` model (`packages/aoa-fastapi-adapter/tests/test_resolve_contract.py`) and by the generated zod `ResolveResponseSchema` (`packages/aoa-client-js/src/codegen/resolve-contract.test.ts`). Not documentation *about* the two shapes matching — its executable definition: change a verdict's shape on one side and not the other, and one of the two halves goes red. The set is split by filename, which is the only thing keeping both sides on the same partition of the same directory: `resolve_response_invalid_*.json` must be **rejected**, every other `resolve_response_*.json` must parse. The broken fixtures are as mandatory a part of the set as the valid ones: a parser that accepts anything would pass a set of only-valid files completely. One special case is `resolve_response_object_forbidden.json`, which encodes oracle safety directly in the data: a question about a nonexistent object and a question about someone else's object give a byte-identical `FORBIDDEN_OBJECT`, and any difference that later appears between them goes red exactly here. Not to be confused with endpoint-set drift — that's `aoa-codegen --check`, not fixtures.

**`ResolveEngine`** — the interface in `aoa-client-js` (`types.ts`) describing everything the library actually requires of an engine: exactly one method, `resolve(items, opts?)`. It doesn't exist for abstraction's sake: `AoaEngine` has private fields, and a private field makes a class type **nominal**, so an object with the same public method set is not assignable to it — however many methods match. While the signatures named the class, a test double was physically impossible to substitute (`makeGatePrimitive`, the generated `createGateApi`/`createApi`, `buildDynamicGateApi`). Deliberately narrow: `cachePartition`, `loadFrom()` and the cache stay on the concrete class — nothing between an engine and a primitive reads them, and the interface can be widened later, never narrowed. `AoaEngine` declares `implements ResolveEngine` so a signature change fails at the class itself rather than at every call site -- with the caveat that this covers a renamed or missing method and a changed return type, but NOT a narrowed parameter (methods are bivariant in their parameters in TypeScript); a dedicated parameter-tuple check in `primitive.types.test.ts` covers that.

**`createMockAoaEngine`** — the engine test double, from the separate `aoa-client-js/testing` entry point. The test author writes a function receiving **one** `ResolveItem` and returning **one** `Verdict`; the double walks the batch, calls it per question, and collects answers of the **same length and in the same order** (every consumer above reads answer N as the answer to question N). The function's second argument is `askedCount` — the number of questions answered before this one, across the double's whole lifetime; that's what makes "allowed a moment ago, denied now" expressible without a counter inside the test. The questions asked are recorded in `engine.calls` together with `opts`, so a test can prove `run()`'s precheck really did ask again (`skipCache: true`). Alongside it, three ready-made verdicts: `success()`, `denied(reason)`, `resolveError(reason)`; both failing ones require a non-empty reason, because the real engine treats an empty one as a broken response, and a double able to build the impossible would hollow out the test. Not to be confused with a fake `fetchImpl` on a real `AoaEngine`: that replaces only the network and is what you want when the library itself is under test; `createMockAoaEngine` replaces the engine wholesale and is what you want when the code ABOVE the library is under test.
