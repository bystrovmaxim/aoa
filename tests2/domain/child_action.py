...
Аспект process принимает строковое значение из params и возвращает
...
class ChildAction(BaseAction["ChildAction.Params", "ChildAction.Result"]):
    ...
    Конвейер:
    1. process (regular) — добавляет префикс "processed:" к значению.
       Чекер: result_string("processed_value", required=True).
    2. build_result (summary) — формирует Result из state.
    """
