...
консольный логер и скоуп — обёртку над словарём переменных,
описывающую местоположение в конвейере выполнения.

Координатор принимает сообщения через метод emit, выполняет
подстановку переменных из контекста выполнения (context, params,
state, scope, var) и рассылает результат по всем зарегистрированным
логерам. Каждый логер самостоятельно решает через match_filters,
нужно ли ему обрабатывать сообщение.
...
LogCoordinator. Если логер сломан, система должна упасть
громко и немедленно. Разработчик узнает о проблеме в момент
её возникновения, а не через месяц когда нужны логи которых нет.
