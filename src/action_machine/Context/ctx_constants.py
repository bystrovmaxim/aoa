...
Константы строго соответствуют реальным полям dataclass'ов UserInfo,
...
═══════════════════════════════════════════════════════════════════════════════
    ...
    Ctx.User      → поля UserInfo (user_id, roles, extra)
...
с дополнительными полями. Для доступа к таким полям используются
строковые пути напрямую:

    @context_requires(Ctx.User.user_id, "user.billing_plan")

Константы Ctx покрывают стандартные поля с автодополнением IDE.
Кастомные поля указываются строками — это осознанный компромисс
между типобезопасностью и расширяемостью.

═══════════════════════════════════════════════════════════════════════════════
ДОСТУП К ВЛОЖЕННЫМ ДАННЫМ EXTRA
═══════════════════════════════════════════════════════════════════════════════

Константы Ctx.User.extra, Ctx.Request.extra и Ctx.Runtime.extra
указывают на словарь extra целиком. Для доступа к конкретному ключу
внутри extra используется строковый путь:

    @context_requires("user.extra.billing_plan")
    @context_requires("request.tags.ab_variant")
...
class _UserFields:
    ...
    соответствующая реальному полю dataclass UserInfo.
    """
    ...
    extra: str = "user.extra"
    """Словарь дополнительных данных. Тип в UserInfo: dict[str, Any]."""
...
class _RequestFields:
    ...
    соответствующая реальному полю dataclass RequestInfo.
    """
    ...
    extra: str = "request.extra"
    """Словарь дополнительных данных. Тип в RequestInfo: dict[str, Any]."""

    tags: str = "request.tags"
    """Произвольные теги запроса. Тип в RequestInfo: dict[str, str]."""
...
class _RuntimeFields:
    ...
    соответствующая реальному полю dataclass RuntimeInfo.
    """
    ...
    extra: str = "runtime.extra"
    """Словарь дополнительных данных. Тип в RuntimeInfo: dict[str, Any]."""
