...
"""
ContextView — frozen-объект с контролируемым доступом к полям контекста.
...
разрешённых ключей. Предоставляет единственный публичный метод get(key),
...
Аспект получает доступ ровно к тем полям контекста, которые объявил
через @context_requires. Обращение к любому другому полю — немедленный
ContextAccessError. Это:

- Делает зависимости от контекста явными и видимыми в коде.
...
Это гарантирует, что аспект не может случайно изменить контекст
или состояние ContextView.
...
вложенные объекты через dot-path. Если поле не существует в контексте,
...
class ContextView:
    """
    Frozen-объект с контролируемым доступом к полям контекста.
    ...
    публичный метод get(key) для чтения разрешённых полей.

    Атрибуты (приватные, записываются через object.__setattr__):
        _context : Any
            Полный контекст выполнения. Используется для резолва значений.
    ...
    def get(self, key: str) -> Any:
        """
        Возвращает значение поля контекста по ключу (dot-path).

        Проверяет, что ключ входит в множество разрешённых. Если да —
        делегирует в context.resolve(key) и возвращает результат.
        Если нет — выбрасывает ContextAccessError.

        Если ключ разрешён, но поле не существует в контексте
        ...
        Возвращает:
            Значение поля из контекста. None если поле не существует.
    ...
    def allowed_keys(self) -> frozenset[str]:
        """
        Возвращает множество разрешённых ключей (только чтение).

        Полезно для отладки и интроспекции: позволяет увидеть,
        какие поля контекста доступны в текущем аспекте.
        """
