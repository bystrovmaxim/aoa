# src/action_machine/compensate/compensate_intent.py
"""
CompensateIntent — маркерный миксин для декоратора @compensate.
...
``require_compensate_intent_marker`` в этом модуле).


AI-CORE-BEGIN
ROLE: module compensate_intent
...
def require_compensate_intent_marker(
    ...
    """Есть @compensate → класс должен наследовать CompensateIntent."""
    if compensators and not issubclass(cls, CompensateIntent):
        ...
        raise TypeError(
            ...
            f"но не наследует CompensateIntent. Декоратор @compensate "
            f"входит в грамматику только при намерении CompensateIntent в MRO. "
            f"Используйте BaseAction или добавьте CompensateIntent "
    ...
    for comp in compensators:
        ...
        if target_aspect.aspect_type != "regular":
            raise ValueError(
                ...
                f"'{target_aspect.aspect_type}'. В грамматике компенсаторы "
                f"привязываются только к regular-аспектам. Summary-аспект формирует "
