# src/action_machine/on_error/on_error_intent.py
"""
Модуль: OnErrorIntent — маркерный миксин для декоратора @on_error.

═══════════════════════════════════════════════════════════════════════════════
НАЗНАЧЕНИЕ
═══════════════════════════════════════════════════════════════════════════════

OnErrorIntent — миксин-маркер, обозначающий, что класс поддерживает
декоратор @on_error для объявления обработчиков неперехваченных исключений
в аспектах. Наследуется BaseAction.

Наличие OnErrorIntent в MRO класса документирует контракт:
...
═══════════════════════════════════════════════════════════════════════════════
    ...
    class BaseAction[P, R](
        ...
        AspectIntent,
        ConnectionIntent,
        OnErrorIntent,                ← маркер: разрешает @on_error
    ): ...
    ...
    #   }
    #
    # OnErrorIntentInspector собирает снимок error_handler:
...
if TYPE_CHECKING:
    from action_machine.on_error.on_error_intent_inspector import (
...
class OnErrorIntent:
    ...
    Класс, наследующий OnErrorIntent, может содержать методы,
    декорированные @on_error для обработки неперехваченных исключений
    в аспектах. Снимок строит ``OnErrorIntentInspector``;
    ...
    гейт-миксинами (RoleIntent, AspectIntent, CheckerIntent и др.).
    ...
    Атрибуты уровня класса (создаются динамически декоратором на методах):
        method._on_error_meta : dict
            ...
            инспектором ``OnErrorIntentInspector`` при построении snapshot.
    """
...
def require_on_error_intent_marker(
    ...
    """Есть @on_error → класс должен наследовать OnErrorIntent."""
    if error_handlers and not issubclass(cls, OnErrorIntent):
        ...
        raise TypeError(
            ...
            f"но не наследует OnErrorIntent. Декоратор @on_error разрешён "
            f"только на классах, наследующих OnErrorIntent. "
            f"Используйте BaseAction или добавьте OnErrorIntent "
