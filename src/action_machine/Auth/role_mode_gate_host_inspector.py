# src/action_machine/auth/role_mode_gate_host_inspector.py
"""
...
Emit a dedicated facet node for every ``RoleModeGateHost`` subclass that carries
``_role_mode_info`` (written by ``@role_mode``). This facet is **separate** from
the action ``role`` facet produced by ``RoleGateHostInspector`` so tooling can
...
- Traversal uses ``_target_mixin = RoleModeGateHost`` (same subtree as
...
::
    ...
    cls._role_mode_info["mode"] == RoleMode.ALIVE
          │
          ▼
    RoleModeGateHostInspector.inspect(cls)
          │
          ▼
...
Edge case: undecorated ``RoleModeGateHost`` subclass without scratch →
...
INVARIANTS: Walk ``RoleModeGateHost``; skip classes without scratch.
...
class RoleModeGateHostInspector(BaseGateHostInspector):
    """
    Builds ``role_mode`` facet nodes from ``@role_mode`` scratch on role types.
    """

    _target_mixin: type = RoleModeGateHost
    ...
    class Snapshot(BaseFacetSnapshot):
        ...
        def to_facet_payload(self) -> FacetPayload:
            return FacetPayload(
                node_type="role_mode",
                node_name=RoleModeGateHostInspector._make_node_name(self.class_ref),
                node_class=self.class_ref,
                node_meta=RoleModeGateHostInspector._make_meta(mode=self.mode),
                edges=(),
            )

        @classmethod
        def from_target(cls, target_cls: type) -> RoleModeGateHostInspector.Snapshot:
    ...
    def inspect(cls, target_cls: type) -> FacetPayload | None:
        if not issubclass(target_cls, RoleModeGateHost):
            return None
    ...
    def facet_snapshot_for_class(
        cls, target_cls: type,
    ) -> RoleModeGateHostInspector.Snapshot | None:
