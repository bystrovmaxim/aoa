...
class RegularAspectGraphNode(BaseCallableGraphNode):
    ...
    CONTRACT: ``node_id`` = ``Introspection.full_qualname(action_cls) + ':' + method_name`` from :class:`BaseCallableGraphNode` resolvers; :attr:`NODE_TYPE` matches facet ``RegularAspect``; empty ``properties`` and ``edges``.
    AI-CORE-END
    """
