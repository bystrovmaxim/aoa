# src/action_machine/model/graph_model/required_context_graph_node.py
"""
...
Materializes a frozen :class:`~graph.base_graph_node.BaseGraphNode` for a single declared
context key on a concrete ``BaseAction`` regular aspect callable. Composition edges to
these nodes originate on :class:`~action_machine.model.graph_model.regular_aspect_graph_node.RegularAspectGraphNode`
(analogous to :class:`~action_machine.model.graph_model.checker_graph_node.CheckerGraphNode` rows bound via ``_checker_meta``).
