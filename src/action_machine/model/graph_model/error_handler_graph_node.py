...
class ErrorHandlerGraphNode(BaseGraphNode[Callable[..., Any]]):
    ...
    CONTRACT: ``node_id`` = ``Introspection.full_qualname(action_cls) + ':' + method_name``; :attr:`NODE_TYPE` matches facet ``error_handler``; empty ``properties`` and ``edges``.
    AI-CORE-END
    """
    ...
    def __init__(self, handler_func: Callable[..., Any]) -> None:
        action_cls = Introspection.owner_type_for_method(handler_func)
        method_name = Introspection.unwrapped_callable_name(handler_func)
