...
class CompensatorGraphNode(BaseGraphNode[Callable[..., Any]]):
    ...
    CONTRACT: ``node_id`` = ``TypeIntrospection.full_qualname(action_cls) + ':' + method_name``; :attr:`NODE_TYPE` matches facet ``Compensator``; ``properties`` include ``description`` when ``CompensateIntentResolver.resolve_description(...)`` returns it; ``edges`` empty.
    AI-CORE-END
    """
