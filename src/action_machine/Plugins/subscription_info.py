...
сохраняется в атрибуте method._on_subscriptions. MetadataBuilder собирает
подписки в ClassMetadata.subscriptions (tuple[SubscriptionInfo, ...]).
...
Порядок проверки фильтров:
    ...
    6. domain             — metadata.meta.domain is domain
...
class SubscriptionInfo:
    ...
    Атрибуты:
        ...
        domain: фильтр по бизнес-домену действия. None — без фильтрации.
            Проверяется через GateCoordinator: metadata.meta.domain is domain.
            Пример: OrdersDomain.
