...
Каждый вызов ``ActionProductMachine._run_internal`` (включая вложенный
``run_child`` → снова ``_run_internal``) вызывает ``create_run_context()`` и
получает **свой** ``PluginRunContext``. Параллельные корневые ``run()`` (через
``asyncio.gather``) тоже используют разные контексты. Состояние обработчиков
внутри одного уровня вложенности не смешивается с родительским уровнем.
