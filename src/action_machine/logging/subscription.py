...
- ``matches`` assumes ``var`` already passed ``LogCoordinator`` validation.
...
Construction-time validation runs in ``LogSubscription.__post_init__``
(channels, levels, domains, non-empty ``key``). ``BaseLogger.subscribe`` only
enforces unique keys among registered subscriptions.
