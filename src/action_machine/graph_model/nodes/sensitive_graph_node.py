# src/action_machine/graph_model/nodes/sensitive_graph_node.py
"""
...
Materializes a frozen :class:`~graph.base_graph_node.BaseGraphNode` for one property
that carries ``_sensitive_config`` (see :mod:`action_machine.intents.sensitive.sensitive_decorator`).
Masking parameters are read via :meth:`~action_machine.intents.sensitive.sensitive_intent_resolver.SensitiveIntentResolver.resolve_sensitive_field`.
``node_id`` uses a ``:sensitive:`` segment so it does not collide with :class:`~action_machine.graph_model.nodes.property_field_graph_node.PropertyFieldGraphNode`
...
class SensitiveGraphNode(BaseGraphNode[SensitiveGraphPayload]):
    ...
    :meth:`~action_machine.intents.sensitive.sensitive_intent_resolver.SensitiveIntentResolver.resolve_sensitive_field`;
