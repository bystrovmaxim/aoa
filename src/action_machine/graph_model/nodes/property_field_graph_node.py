...
If the member is ``@sensitive`` on ``parent_type``, a stub :class:`~action_machine.graph_model.edges.sensitive_graph_edge.SensitiveGraphEdge`
is resolved internally; :meth:`get_all_edges` returns it, and :meth:`get_companion_nodes` adds the matching :class:`~action_machine.graph_model.nodes.sensitive_graph_node.SensitiveGraphNode`.

Constructor shape matches :class:`FieldGraphNode` (``parent_type``, name, ``required``); this node names that string ``property_name`` on the payload. Callers supply interchange metadata explicitly; optional stub edges use :meth:`~action_machine.graph_model.edges.sensitive_graph_edge.SensitiveGraphEdge.get_sensitive_edge`.
"""
...
from action_machine.graph_model.edges.sensitive_graph_edge import SensitiveGraphEdge
...
class PropertyFieldGraphPayload:
    """Frozen payload for :attr:`~graph.base_graph_node.BaseGraphNode.node_obj` (property-field row metadata)."""
...
class PropertyFieldGraphNode(BaseGraphNode[PropertyFieldGraphPayload]):
    ...
    ``@sensitive`` stub edge is derived via :meth:`~action_machine.graph_model.edges.sensitive_graph_edge.SensitiveGraphEdge.get_sensitive_edge`;
    when non-``None``, :meth:`get_companion_nodes` materializes :class:`~action_machine.graph_model.nodes.sensitive_graph_node.SensitiveGraphNode`;
