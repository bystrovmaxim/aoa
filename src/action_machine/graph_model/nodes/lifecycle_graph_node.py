...
class LifeCycleGraphNode(BaseGraphNode[LifeCycleGraphPayload]):
    """
    AI-CORE-BEGIN
    ROLE: Interchange vertex for one lifecycle-declared slot on ``host_cls``.
    CONTRACT: ``node_id`` is ``full_qualname(host_cls) + ':lifecycle:' + field_name.strip()``; ``label`` trimmed field key; ``lifecycle_class`` supplied by caller; ``properties`` empty on construction; ``node_obj`` is :class:`LifeCycleGraphPayload`.
    INVARIANTS: Frozen; emits no outgoing edges by default (:meth:`~graph.base_graph_node.BaseGraphNode.get_all_edges`).
