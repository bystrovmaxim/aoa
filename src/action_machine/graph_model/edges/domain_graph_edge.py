...
target is :class:`~action_machine.graph_model.nodes.domain_graph_node.DomainGraphNode`.
...
class DomainGraphEdge(AssociationGraphEdge):
    ...
    FAILURES: :exc:`~action_machine.exceptions.MissingMetaError` when ``resolve_domain_type`` cannot read a ``BaseDomain`` from ``@meta``.
    AI-CORE-END
    """
