# src/action_machine/graph_model/edges/lifecycle_state_graph_edge.py
"""
LifeCycleStateGraphEdge — COMPOSITION from a lifecycle row to one state row.

Keeps every :class:`~action_machine.graph_model.nodes.state_graph_node.StateGraphNode`
visibly attached to its parent :class:`~action_machine.graph_model.nodes.lifecycle_graph_node.LifeCycleGraphNode`,
...
class LifeCycleStateGraphEdge(CompositionGraphEdge):
    ...
    CONTRACT: ``edge_name`` ``lifecycle_contains_state``; ``is_dag`` False; target is a wired :class:`~action_machine.graph_model.nodes.state_graph_node.StateGraphNode`.
