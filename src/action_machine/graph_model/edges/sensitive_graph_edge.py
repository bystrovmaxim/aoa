# src/action_machine/graph_model/edges/sensitive_graph_edge.py
"""
...
Typed composition edge whose ``target_node`` is a :class:`~action_machine.graph_model.nodes.sensitive_graph_node.SensitiveGraphNode`
...
from action_machine.graph_model.nodes.sensitive_graph_node import SensitiveGraphNode, SensitiveGraphNodeEdgeInfo
...
class SensitiveGraphEdge(CompositionGraphEdge):
    ...
    CONTRACT: ``edge_name`` literal ``sensitive``; ``is_dag`` False; pass wired ``sensitive_node`` or stub ``target_node_id``;
        ...
        :meth:`get_sensitive_edges` builds edges from :meth:`~action_machine.intents.sensitive.sensitive_intent_resolver.SensitiveIntentResolver.resolve_sensitive_all_fields`.
