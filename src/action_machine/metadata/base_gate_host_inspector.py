...
"""
BaseGateHostInspector — abstract base for every ActionMachine gate-host inspector.

═══════════════════════════════════════════════════════════════════════════════
PURPOSE
═══════════════════════════════════════════════════════════════════════════════

``BaseGateHostInspector`` defines the contract every concrete inspector must
follow.

An inspector:
1. Knows which marker mixin subtree to walk (``_target_mixin``).
...
Each gate host concept exists twice:

    Marker mixin (``RoleGateHost``, ``AspectGateHost``, ``DependencyGateHost[T]``, …)
        Lives in the MRO of ``BaseAction`` (or ``BaseEntity``, ``BaseResourceManager``).
        Enables the matching decorator via ``issubclass`` checks.
        Contains no inspection logic. Does not inherit ``BaseGateHostInspector``.
        Stays stable across refactors.

    Inspector (``RoleGateHostInspector``, ``AspectGateHostInspector``, …)
        Inherits ``BaseGateHostInspector``. Implements ``inspect()`` and
        ``_build_payload()``. Walks marker subclasses via ``_target_mixin``.
        Registers with the coordinator.

The link is the inspector's ``_target_mixin`` (single marker) or, rarely, the
``_target_mixins`` tuple / custom ``_subclasses_recursive()`` used by
``MetaGateHostInspector`` to union action and resource markers without duplicate
...
itself. Concrete inspectors override it to traverse ``_target_mixin`` instead::
...
class BaseGateHostInspector(ABC):
    ...
    def _subclasses_recursive(cls) -> list[type]:
        ...
        Concrete inspectors override it to scan ``_target_mixin`` instead::
