...
- ``create`` receives all runtime values explicitly; no ``_MachineLike`` protocol.
- Public ``create(...)`` API remains stable aside from replacing the first-arg
  machine with ``factory_resolver`` + ``mode`` + ``machine_class_name``.
...
from action_machine.core.components.dependency_factory_resolver import (
