...
_required_context_keys). MetadataBuilder собирает эти атрибуты
...
═══════════════════════════════════════════════════════════════════════════════
КОНТЕКСТНЫЕ ЗАВИСИМОСТИ АСПЕКТОВ И ОБРАБОТЧИКОВ
═══════════════════════════════════════════════════════════════════════════════

Поле context_keys в AspectMeta и OnErrorMeta содержит frozenset строковых
ключей (dot-path), объявленных через @context_requires. Машина
(ActionProductMachine) использует это поле при вызове аспекта:

    Если context_keys непустой:
        → создаёт ContextView(context, context_keys)
        → передаёт как последний аргумент (ctx)
        → сигнатура аспекта: (self, params, state, box, connections, ctx)

    Если context_keys пустой:
        → ctx не передаётся
        → сигнатура аспекта: (self, params, state, box, connections)
