...
2. ПОЛНОТА: ClassMetadata содержит ВСЮ информацию, необходимую для выполнения
   действия — описание (@meta), роли, зависимости, соединения, аспекты,
   чекеры, подписки, чувствительные поля.
...
═══════════════════════════════════════════════════════════════════════════════
    ...
    if metadata.has_meta():
        print(f"Описание: {metadata.meta.description}")
        if metadata.meta.domain:
            print(f"Домен: {metadata.meta.domain.name}")
...
class MetaInfo:
    ...
    Содержит обязательное описание класса и опциональную привязку к домену.
    Описание используется в графе координатора, в логах, в интроспекции
    и в адаптерах (FastAPI, MCP) для автоматической генерации документации.

    Поля:
        description : str
            Текстовое описание класса. Что делает действие или ресурсный
            менеджер. Непустая строка. Обязательна для всех классов
            с ActionMetaGateHost или ResourceMetaGateHost.

        domain : type[BaseDomain] | None
            Класс бизнес-домена, к которому принадлежит класс.
            None если привязка к домену не указана. В strict-режиме
            координатора domain обязателен.
    """
    description: str
    domain: Any = None  # type[BaseDomain] | None, Any для избежания циклических импортов
...
class ClassMetadata:
    ...
    Поля:
        ...
        meta : MetaInfo | None
            Описание и доменная принадлежность (от @meta). None если декоратор
            не применялся. Для классов с ActionMetaGateHost или
            ResourceMetaGateHost — обязательно (проверяется MetadataBuilder).
