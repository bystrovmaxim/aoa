...
осуществляется через шлюзы (gates), которые присоединяются к классу через миксины:
...
- ConnectionGateHost – для соединений (декоратор @connection)
...
Пример:
    ...
    ... @connection("db", PostgresConnectionManager, description="Основная БД")
...
class BaseAction(
    ...
    Метаданные (роли, зависимости, чекеры, аспекты, соединения) собираются автоматически
    ...
    - get_connection_gate() → ConnectionGate
    ...
    # Старый атрибут _connections удалён, теперь используется ConnectionGate.
    # Для обратной совместимости он больше не поддерживается.
