...
═══════════════════════════════════════════════════════════════════════════════
ПРИВАТНОСТЬ КОНТЕКСТА — КЛЮЧЕВОЙ INVARIANT
═══════════════════════════════════════════════════════════════════════════════

ToolsBox НЕ предоставляет доступа к contextу выполнения (Context).
Публичного свойства ``context`` нет. Публичного methodа, возвращающего
Context, нет. Ни один публичный атрибут или method не раскрывает Context.
...
Единственный легальный путь к данным contextа для аспекта —
через ``ContextView``, который машина (ActionProductMachine) создаёт
для methodов с декоратором ``@context_requires`` и передаёт как
...
class ToolsBox:
    ...
    def __init__(
        ...
        Args:
            ...
            context: context выполнения (приватный).
    ...
    async def run(
        ...
        """
        Запускает дочернее действие с текущими ресурсами и contextом.
        """
