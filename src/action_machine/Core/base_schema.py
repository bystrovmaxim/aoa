...
выполнения), наследуют от BaseSchema.
...
1. ВСЁ FROZEN. Каждый наследник неизменяем после создания. Единственный
   способ «изменить» — создать новый экземпляр.
...
2. ВСЁ FORBID (кроме BaseState). Произвольные поля запрещены. Расширение
   ...
   BaseState использует extra="allow" для динамических полей конвейера.

3. ЕДИНЫЙ ИНТЕРФЕЙС. Dict-подобный доступ и dot-path навигация доступны
   на любой схеме. Один базовый класс вместо отдельных Protocol и Mixin.
...
возвращают как объявленные, так и динамические extra-поля.
...
class BaseSchema(BaseModel):
    ...
    def resolve(self, dotpath: str, default: object = None) -> object:
        ...
        На каждом шаге выбирает стратегию навигации по типу текущего
        объекта:
