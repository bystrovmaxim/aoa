...
Декоратор @meta применяется к двум различным иерархиям классов:

1. Действия (Action) — наследники BaseAction.
2. Ресурсные менеджеры (ResourceManager) — наследники BaseResourceManager.

Эти иерархии не пересекаются. Для каждой создан свой маркерный миксин,
...
Декоратор @meta при применении проверяет, что целевой класс наследует
...
2. МАРКЕРЫ БЕЗ ЛОГИКИ. Миксины не содержат полей, методов или логики.
...
class ActionMetaGateHost:
    """
    Маркерный миксин, обозначающий обязательность декоратора @meta
    для классов действий (Action).

    Наследуется BaseAction. Класс, наследующий ActionMetaGateHost
    и содержащий аспекты, обязан иметь декоратор @meta с описанием.
    MetadataBuilder проверяет это при сборке ClassMetadata.
    ...
    Атрибуты уровня класса (создаются динамически декоратором @meta):
        _meta_info : dict[str, Any]
            ...
            при сборке ClassMetadata.meta (MetaInfo).
    """
...
class ResourceMetaGateHost:
    ...
    Наследуется BaseResourceManager. Класс, наследующий
