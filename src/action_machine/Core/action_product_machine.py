...
Координатор НЕ создаётся внутри конструктора безусловно — это позволяет
легко передать уже существующий координатор с нужной конфигурацией
(например, strict=True для production).

    # Координатор по умолчанию (strict=False):
    machine = ActionProductMachine(mode="production")

    # Передача существующего координатора:
    coordinator = GateCoordinator(strict=True)
    machine = ActionProductMachine(mode="production", coordinator=coordinator)

    # Разделение координатора между машинами:
    shared = GateCoordinator()
    prod_machine = ActionProductMachine(mode="prod", coordinator=shared)
    test_machine = ActionTestMachine(coordinator=shared)
...
class ActionProductMachine(BaseActionMachine):
    ...
    Атрибуты:
        ...
        _coordinator : GateCoordinator
            Координатор метаданных и фабрик. Передаётся извне или
            создаётся с настройками по умолчанию.
    ...
    def __init__(
        ...
        Аргументы:
            ...
            coordinator: координатор метаданных и фабрик. Если не указан,
                         создаётся новый GateCoordinator() с настройками
                         по умолчанию (strict=False). Передача существующего
                         координатора позволяет настроить strict-режим,
                         разделить координатор между машинами и тестами.
