...
7. При исключении в аспекте — эмитирует событие "on_error" плагинам,
   затем ищет подходящий обработчик @on_error.
...
2. Эмитирует событие "on_error" плагинам — плагины-наблюдатели получают
   информацию об ошибке, включая тип исключения, имя действия и наличие
   подходящего обработчика @on_error на уровне Action. Плагины НЕ могут
   изменить результат или подавить ошибку — они только наблюдают.
3. Читает metadata.error_handlers — обработчики @on_error в порядке
   объявления (сверху вниз).
4. Ищет первый подходящий обработчик через isinstance(error, handler.exception_types).
5. Если подходящий найден — вызывает handler(action, params, state, box, connections, error).
   - Если обработчик возвращает Result — ошибка обработана, Result подменяет
     результат действия.
   - Если обработчик сам бросает исключение — оно оборачивается
     в OnErrorHandlerError с __cause__ и пробрасывается наружу.
6. Если ни один обработчик не подошёл — исходное исключение летит наружу
   как необработанное.
...
═══════════════════════════════════════════════════════════════════════════════
СОБЫТИЕ on_error ДЛЯ ПЛАГИНОВ
═══════════════════════════════════════════════════════════════════════════════

Событие "on_error" эмитируется ПЕРЕД вызовом обработчика @on_error
на уровне Action. Это позволяет плагинам-наблюдателям:
- Считать ошибки (метрики).
- Логировать ошибки в централизованное хранилище.
- Отправлять алерты.

Плагин получает PluginEvent с полями:
    event_name          = "on_error"
    action_name         = полное имя действия
    error               = экземпляр исключения
    has_action_handler  = True если у Action есть подходящий @on_error
...
class ActionProductMachine(BaseActionMachine):
    ...
    применяет чекеры к результатам аспектов, эмитирует событие "on_error"
    ...
    async def _handle_aspect_error(
        ...
        через isinstance, вызывается с параметрами:
        (action, params, state, box, connections, error).

        Если обработчик возвращает Result — ошибка считается обработанной.
        Если обработчик сам бросает исключение — оно оборачивается
        в OnErrorHandlerError с __cause__.
        Если ни один обработчик не подходит — исходное исключение
        пробрасывается через raise.
    ...
    async def _execute_regular_aspects(
        ...
        наверх в _execute_aspects_with_error_handling(), где эмитируется
        событие "on_error" плагинам и ищется подходящий обработчик @on_error.
        """
        state = BaseState()
    ...
    async def _execute_aspects_with_error_handling(
        ...
        Оборачивает выполнение regular-аспектов и summary-аспекта в try/except.
        При исключении:
        1. Эмитирует событие "on_error" плагинам — плагины-наблюдатели получают
        ...
        3. Если обработчик найден и вернул Result — возвращает его.
        4. Если обработчик не найден или сам бросил исключение —
        ...
        Возвращает:
            R — результат действия (от summary-аспекта или от обработчика ошибки).
        """
        try:
            ...
            # Плагины-наблюдатели получают информацию об ошибке ДО вызова
            # обработчика @on_error на уровне Action. Они не могут
            # изменить результат или подавить ошибку — только наблюдают.
            plugin_kwargs = self._build_plugin_emit_kwargs(box.nested_level)
            ...
            # State на момент ошибки: если ошибка в regular-аспектах,
            # state может быть частично заполнен. Для простоты передаём
            # пустой BaseState — обработчик не должен полагаться на state.
