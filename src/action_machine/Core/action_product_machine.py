...
Машина не хранит никакого состояния между вызовами run(), связанного
с зависимостями. Поле _factory_cache удалено. Фабрики зависимостей
хранятся в координаторе (GateCoordinator._factory_cache) и являются
stateless — каждый resolve() создаёт свежий экземпляр, если пользователь
не указал lambda-синглтон в @depends.

Каждый вызов run() изолирован от предыдущих:
- Метаданные получаются из координатора (кешируются там, не здесь).
- Фабрика получается из координатора (кешируется там, не здесь).
...
Координатор передаётся в конструктор машины (dependency injection).
...
class ActionProductMachine(BaseActionMachine):
    ...
    Машина не хранит никакого состояния между вызовами run(), связанного
    с зависимостями — каждый вызов полностью изолирован.
