...
принимает rollup как обязательный параметр без значения по умолчанию
в терминальных методах — тестировщик явно выбирает режим.
...
class BaseActionMachine(ABC):
    ...
    Определяет контракт: публичный метод run() (абстрактный)
