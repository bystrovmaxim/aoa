...
- Объявляет атрибут _field_checkers для использования декораторами чекеров.
...
Механизм сбора:
    Чекеры для класса: собираются из атрибута _field_checkers (заполняется декораторами
    чекеров, применёнными к классу). Каждый чекер регистрируется в шлюзе с target_type='class'.
...
Важно:
    ...
    Атрибут _field_checkers объявлен как ClassVar и инициализирован пустым списком.
    Декораторы чекеров добавляют себя в этот список. __init_subclass__ собирает
    чекеры из _field_checkers и регистрирует их в шлюзе.
"""
...
class CheckerGateHost:
    ...
    Классовые атрибуты:
        ...
        _field_checkers: list[BaseFieldChecker] – список классовых чекеров,
                         заполняемый декораторами. Объявлен здесь, чтобы
                         декораторы не создавали динамических атрибутов.
    """

    __checker_gate: ClassVar[CheckerGate | None] = None
    _field_checkers: ClassVar[list[BaseFieldChecker]] = []
