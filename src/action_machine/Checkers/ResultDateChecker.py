"""
Чекер для полей с датой/временем результата аспекта.
...
Использование:
    Применяется как декоратор к методам-аспектам:
        @regular_aspect("Создание записи")
        @ResultDateChecker("created_at", "Дата создания", required=True, format="%Y-%m-%d")
