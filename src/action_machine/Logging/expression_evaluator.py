...
- Функция exists(name): возвращает True если переменная с указанным
  ...
  Полезна для условного логирования, когда переменная может отсутствовать.

Все переменные из контекста выполнения (var, state, params, context, scope)
доступны внутри выражений как литеральные значения, подставленные
координатором ДО вычисления iif.
...
ANSI-коды в VariableSubstitutor.

═══════════════════════════════════════════════════════════════════════════════
СОВМЕСТИМОСТЬ С PYDANTIC V2.11+
═══════════════════════════════════════════════════════════════════════════════

Pydantic V2.11 помечает обращение к model_fields, model_computed_fields
и другим class-level атрибутам через экземпляр как deprecated. Функции
интроспекции (_is_public_data_attribute, _is_public_property) фильтруют
эти атрибуты через множество _PYDANTIC_CLASS_ATTRS, чтобы избежать
DeprecationWarning при вызове getattr(obj, name).
...
Разбор аргументов iif выполняется отдельным классом _IifArgSplitter,
...
# Использует нежадный захват с последующей проверкой баланса скобок.
...
# Фильтруются в _is_public_data_attribute и _is_public_property.
...
def _is_pydantic_class_attr(obj: Any, name: str) -> bool:
    ...
    DeprecationWarning в Pydantic V2.11+. Функция возвращает True
    для всех имён из _PYDANTIC_CLASS_ATTRS, если объект является
    экземпляром pydantic BaseModel.
...
def _is_public_data_attribute(obj: Any, name: str) -> bool:
    ...
    - Методы (callable).
    - Pydantic class-level атрибуты (model_fields, model_computed_fields и т.д.).
...
def _format_field_line(
    ...
    Возвращает одну строку (если глубина рекурсии исчерпана или значение
    примитивное) или строку плюс рекурсивно развёрнутый блок.
    """
...
_BASE_SAFE_FUNCTIONS: dict[str, Any] = {
    ...
    # Цветовые функции — оборачивают аргумент в маркер
    ...
    # Функция debug — возвращает форматированную строку для интроспекции
...
class _IifArgSplitter:
    ...
    def _handle_string_char(self, char: str) -> bool:
        ...
        Единственный особый символ — закрывающая кавычка (совпадающая
        с открывающей), которая завершает строковый режим.
...
class ExpressionEvaluator:
    ...
    def evaluate_iif(
        ...
        Поддерживает вложенные iif — если выбранная ветка начинается
        с "iif(", она обрабатывается рекурсивно.
