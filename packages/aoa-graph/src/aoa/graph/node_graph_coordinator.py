...
a ``rustworkx.PyDiGraph`` whose node weights are :class:`~aoa.graph.base_graph_node.BaseGraphNode`
instances and edge weights are :class:`~aoa.graph.base_graph_edge.BaseGraphEdge` instances.
...
class NodeGraphCoordinator:
    ...
    def rx_graph(self) -> rx.PyDiGraph:
        ...
        Node weights are :class:`~aoa.graph.base_graph_node.BaseGraphNode` payloads (same order as sorted ``node_id``).
        Edge weights are :class:`~aoa.graph.base_graph_edge.BaseGraphEdge` instances.
    ...
    def get_all_nodes(self) -> tuple[BaseGraphNode[Any], ...]:
        """
        Return every :class:`~aoa.graph.base_graph_node.BaseGraphNode` in graph node order.
    ...
    def _map_unique_node_ids(
        ...
        """
        Build ``node_id -> node`` and ensure each :attr:`~aoa.graph.base_graph_node.BaseGraphNode.node_id`
