...
:class:`~aoa.graph.base_graph_edge.BaseGraphEdge`. :func:`export_interchange_axes_graph_html`
writes a standalone AntV G6 HTML file for an **already built** coordinator to
:data:`HTML_PATH`
(``<workspace>/archive/logs/graph_node.html``; UTF-8, parent directories created as needed).
Markup and layout shell live in ``template.html`` beside this module;
Python injects graph JSON, seeds, and runtime script.
It does **not** call :meth:`~aoa.graph.node_graph_coordinator.NodeGraphCoordinator.build` or change inspectors.
...
def _default_archive_logs_dir() -> Path:
    """Workspace ``archive/logs`` output directory for generated graph artifacts."""
    return archive_logs_dir()
...
def export_interchange_axes_graph_html(
    ...
    *,
    ...
    ``coordinator`` must already have a successful :meth:`~aoa.graph.node_graph_coordinator.NodeGraphCoordinator.build`.
    Output path is always :data:`HTML_PATH`
    (``<workspace>/archive/logs/graph_node.html``).
    """
...
def write_demo_interchange_axes_graph_html() -> Path:
    ...
    from aoa.maxitor.samples.interchange_demo_coordinator import (  # pylint: disable=import-outside-toplevel
