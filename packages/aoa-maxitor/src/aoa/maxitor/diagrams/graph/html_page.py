# packages/aoa-maxitor/src/aoa/maxitor/diagrams/graph/html_page.py
"""
AntV G6 HTML for interchange graphs.

Graph nodes carry :class:`~aoa.graph.base_graph_node.BaseGraphNode` and edges carry
:class:`~aoa.graph.base_graph_edge.BaseGraphEdge`. :func:`interchange_g6_html_string_from_coordinator`
returns a standalone HTML document string for an **already built** coordinator.
:func:`interchange_g6_html_string_from_nx` builds the same viewer from a
:class:`networkx.DiGraph` whose nodes carry ``node_type`` / ``label`` and edges carry
``edge_name`` / ``is_dag`` (as produced by :class:`~aoa.maxitor.model.app_view.actions.load_graph_action.LoadGraphAction`).

Markup and layout shell live in ``template.html`` beside this module;
Python injects graph JSON, seeds, and runtime script.
This module does **not** call :meth:`~aoa.graph.node_graph_coordinator.NodeGraphCoordinator.build` or change inspectors.

:func:`interchange_g6_html_string_from_coordinator` serializes **only**
...
:mod:`~aoa.maxitor.diagrams.graph.domain_propagation` so this module stays maintainable.
...
from aoa.action_machine.graph_model.nodes.regular_aspect_graph_node import (
    RegularAspectGraphNode,
)
from aoa.action_machine.graph_model.nodes.required_context_graph_node import (
...
from aoa.maxitor.diagrams.shared import read_interchange_chrome_css
...
def _assemble_interchange_g6_html(
    g6_nodes: list[dict[str, Any]],
...
def interchange_g6_html_string_from_coordinator(  # pylint: disable=too-many-statements
    ...
    *,
    ...
    for idx, raw in enumerate(nodes_tuple):
        if not isinstance(raw, BaseGraphNode):
            msg = (
                f"interchange_g6_html_string_from_coordinator expects BaseGraphNode payloads on nodes; "
    ...
    return _assemble_interchange_g6_html(
        g6_nodes,
        g6_edges,
        legend_items,
        node_type_map,
...
def interchange_g6_html_string_from_nx(  # pylint: disable=too-many-statements
    nx_graph: nx.DiGraph,
    *,
    title: str = "ActionMachine Graph",
    width: str = "100%",
    ...
    """Build the same G6 HTML document topology from a :class:`networkx.DiGraph` (e.g. :class:`LoadGraphAction` result)."""
