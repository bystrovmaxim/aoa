# packages/aoa-mcp-adapter/tests/support/domain_model.py
"""
Domain model fixtures for the MCP adapter test suite.

════════════════════════════════════════════════════════════════════════════════
PURPOSE
════════════════════════════════════════════════════════════════════════════════

Self-contained copy of the scenario domain model the MCP adapter tests rely on.
Bundles the business domains, the shared application roles, the dependency
services and DB resource manager, the minimal ``SampleEntity`` used for
``BaseEntity.schema()`` wire projections, and the three smoke-test actions:

- ``PingAction``  — no-parameter action returning a fixed ``"pong"`` message
  (registered as ``system.ping`` / ``ping`` tools). GuestRole, SystemDomain.
- ``SimpleAction`` — single required ``name`` parameter, returns a greeting
  (registered as ``simple.run`` in multi-tool / chaining tests). GuestRole,
  OrdersDomain.
- ``FullAction``  — full-featured action with service dependencies, a DB
  connection and a multi-field ``Params`` projection (registered as
  ``orders.create``). ManagerRole, OrdersDomain.
