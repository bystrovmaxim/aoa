# packages/aoa-mcp-adapter/tests/support/adapter_fixtures.py
"""
Shared adapter-fixture actions for the MCP adapter test suite.
...
used to assert MCP ``inputSchema`` projection and raw-dict serialization:
...
- ``EntityProjectionParamsMcpTestAction`` — sibling whose ``Params`` carry the
  same entity wire projection (drives the MCP ``inputSchema`` entity coverage).
