# packages/aoa-action-machine/src/aoa/action_machine/graph/nodes/summary_aspect_graph_node.py
"""
...
Materializes a frozen :class:`~aoa.action_machine.graph.core.base_graph_node.BaseGraphNode` for one summary
...
``@context_requires`` wired as :attr:`required_context` composition edges (:class:`~aoa.action_machine.graph.edges.required_context_graph_edge.RequiredContextGraphEdge`).
...
class SummaryAspectGraphNode(BaseGraphNode[Callable[..., Any]]):
    ...
    CONTRACT: ``node_id`` = ``TypeIntrospection.full_qualname(_action_cls) + ':' + method_name``; :attr:`NODE_TYPE` is ``SummaryAspect``; ``properties`` include ``description`` from :meth:`~aoa.action_machine.intents.aspects.summary_aspect_intent_resolver.SummaryAspectIntentResolver.resolve_description`; :attr:`required_context` via :meth:`~aoa.action_machine.graph.edges.required_context_graph_edge.RequiredContextGraphEdge.get_required_context_edges`; companions are wired ``RequiredContextGraphNode`` targets on those edges.
    AI-CORE-END
    """
