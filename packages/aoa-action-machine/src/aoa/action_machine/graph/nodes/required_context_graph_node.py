# packages/aoa-action-machine/src/aoa/action_machine/graph/nodes/required_context_graph_node.py
"""
...
Materializes a frozen :class:`~aoa.action_machine.graph.core.base_graph_node.BaseGraphNode` for a single declared
context key on a concrete ``BaseAction`` regular aspect callable. Composition edges to
these nodes originate on :class:`~aoa.action_machine.graph.nodes.regular_aspect_graph_node.RegularAspectGraphNode`
(analogous to :class:`~aoa.action_machine.graph.nodes.checker_graph_node.CheckerGraphNode` rows bound via ``_checker_meta``).
