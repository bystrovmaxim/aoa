# packages/aoa-action-machine/src/aoa/action_machine/graph/nodes/error_handler_graph_node.py
"""
...
Materializes a frozen :class:`~aoa.action_machine.graph.core.base_graph_node.BaseGraphNode` for one error-handler
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                **(
                    {
                        "exception_types": [str(x) for x in self.properties["exception_types"]],
                    }
                    if "exception_types" in self.properties and isinstance(self.properties["exception_types"], list)
                    else {}
                ),
            },
        }
