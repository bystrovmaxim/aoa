# packages/aoa-action-machine/src/aoa/action_machine/graph/nodes/property_field_graph_node.py
"""
...
Materializes a frozen :class:`~aoa.action_machine.graph.core.base_graph_node.BaseGraphNode` for one logical property / computed-style member under a
...
If the member is ``@sensitive`` on ``parent_type``, a stub :class:`~aoa.action_machine.graph.edges.sensitive_graph_edge.SensitiveGraphEdge`
is resolved internally; :meth:`get_all_edges` returns it, and :meth:`get_companion_nodes` adds the matching :class:`~aoa.action_machine.graph.nodes.sensitive_graph_node.SensitiveGraphNode`.
...
Constructor shape matches :class:`FieldGraphNode` (``parent_type``, name, ``required``); this node names that string ``property_name`` on the payload. Callers supply interchange metadata explicitly; optional stub edges use :meth:`~aoa.action_machine.graph.edges.sensitive_graph_edge.SensitiveGraphEdge.get_sensitive_edge`.
"""
...
from aoa.action_machine.graph.edges.sensitive_graph_edge import SensitiveGraphEdge
from aoa.action_machine.graph.nodes.sensitive_graph_node import SensitiveGraphNode
from aoa.action_machine.system_core.type_introspection import TypeIntrospection


@dataclass(frozen=True)
class PropertyFieldGraphPayload:
    """Frozen payload for :attr:`~aoa.action_machine.graph.core.base_graph_node.BaseGraphNode.node_obj` (property-field row metadata)."""
...
class PropertyFieldGraphNode(BaseGraphNode[PropertyFieldGraphPayload]):
    ...
    ``@sensitive`` stub edge is derived via :meth:`~aoa.action_machine.graph.edges.sensitive_graph_edge.SensitiveGraphEdge.get_sensitive_edge`;
    when non-``None``, :meth:`get_companion_nodes` materializes :class:`~aoa.action_machine.graph.nodes.sensitive_graph_node.SensitiveGraphNode`;
