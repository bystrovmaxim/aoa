# packages/aoa-action-machine/src/aoa/action_machine/graph/nodes/state_graph_node.py
"""
StateGraphNode — interchange graph node for one state slot under a lifecycle graph node.

``node_id`` is the parent :class:`~aoa.action_machine.graph.nodes.lifecycle_graph_node.LifeCycleGraphNode`
interchange id plus ``:`` and the caller-supplied state key (trimmed).

Outgoing :class:`~aoa.action_machine.graph.edges.state_graph_edge.StateGraphEdge`
rows live in :attr:`lifecycle_transitions` and are returned from :meth:`~aoa.action_machine.graph.core.base_graph_node.BaseGraphNode.get_all_edges`.

The parent :class:`~aoa.action_machine.graph.nodes.lifecycle_graph_node.LifeCycleGraphNode` keeps companion rows in :attr:`~aoa.action_machine.graph.nodes.lifecycle_graph_node.LifeCycleGraphNode.states`; :meth:`~aoa.action_machine.graph.nodes.lifecycle_graph_node.LifeCycleGraphNode.get_all_edges` flattens the **same instances** carried here in :attr:`lifecycle_transitions`.

Constructed only via :meth:`~aoa.action_machine.graph.edges.lifecycle_state_graph_edge.LifeCycleStateGraphEdge.get_state_edges`
...
from aoa.action_machine.graph.edges.state_graph_edge import StateGraphEdge
...
class StateGraphPayload:
    """Frozen payload for :attr:`~aoa.action_machine.graph.core.base_graph_node.BaseGraphNode.node_obj`."""
...
class StateGraphNode(BaseGraphNode[StateGraphPayload]):
    """
    AI-CORE-BEGIN
    ROLE: Interchange graph node for one state key scoped under the parent interchange row :class:`~aoa.action_machine.graph.nodes.lifecycle_graph_node.LifeCycleGraphNode`.
    ...
    INVARIANTS: Frozen; :attr:`lifecycle_transitions` lists transitions from :meth:`~aoa.action_machine.graph.edges.state_graph_edge.StateGraphEdge.get_lifecycle_transition_edges` and is surfaced by ``get_all_edges``; :meth:`get_companion_nodes` is always empty (lifecycle rows register via ``LifeCycleGraphEdge``).
