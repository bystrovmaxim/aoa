# packages/aoa-action-machine/src/aoa/action_machine/graph/node_graph_coordinator_factory.py
"""
...
Build :class:`~aoa.action_machine.graph.core.node_graph_coordinator.NodeGraphCoordinator` with the
standard interchange inspectors under ``aoa.action_machine.graph``. This module
belongs to ``action_machine`` so the ``graph`` shim does not pull domain wiring into ``core``.

Also re-exports :data:`GRAPH_JSON_SCHEMA` for callers (for example Maxitor samples) that
build a coordinator with :meth:`~aoa.action_machine.graph.core.node_graph_coordinator.NodeGraphCoordinator.build`
...
from aoa.action_machine.graph.inspectors.resource_graph_node_inspector import ResourceGraphNodeInspector
from aoa.action_machine.graph.inspectors.result_graph_node_inspector import ResultGraphNodeInspector
