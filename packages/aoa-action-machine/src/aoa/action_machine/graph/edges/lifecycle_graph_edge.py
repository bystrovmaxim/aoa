# packages/aoa-action-machine/src/aoa/action_machine/graph/edges/lifecycle_graph_edge.py
"""
LifeCycleGraphEdge — COMPOSITION from entity (or host) interchange row to one ``Lifecycle`` field graph node.

Wiring ``target_node`` to :class:`~aoa.action_machine.graph.nodes.lifecycle_graph_node.LifeCycleGraphNode`
...
Interchange companions for state graph nodes are chained only via :class:`~aoa.action_machine.graph.nodes.lifecycle_graph_node.LifeCycleGraphNode` reachable from ``LifeCycleGraphEdge.target_node``.
:class:`~aoa.action_machine.graph.nodes.state_graph_node.StateGraphNode` rows never advertise companions.
"""
...
class LifeCycleGraphEdge(CompositionGraphEdge):
    ...
    def get_lifecycle_edges(entity_cls: type[BaseEntity]) -> list[LifeCycleGraphEdge]:
        """``lifecycle`` composition edges (wired :class:`~aoa.action_machine.graph.nodes.lifecycle_graph_node.LifeCycleGraphNode` targets) for every declared field."""
