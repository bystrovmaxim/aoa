# packages/aoa-action-machine/src/aoa/action_machine/graph/edges/checker_graph_edge.py
"""
...
Provides the typed :class:`CheckerGraphEdge` for each :class:`~aoa.action_machine.graph.nodes.checker_graph_node.CheckerGraphNode`
under a :class:`~aoa.action_machine.graph.nodes.regular_aspect_graph_node.RegularAspectGraphNode`
(``edge_name`` ``@result_checker``, ``is_dag`` False), instead of synthesizing a bare
:class:`~aoa.action_machine.graph.core.composition_graph_edge.CompositionGraphEdge`.
...
read ``_checker_meta`` on the aspect callable and attach :class:`~aoa.action_machine.graph.nodes.checker_graph_node.CheckerGraphNode`
