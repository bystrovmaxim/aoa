# packages/aoa-action-machine/src/aoa/action_machine/graph/core/base_graph_edge.py
"""
...
:class:`~aoa.action_machine.graph.core.base_graph_node.BaseGraphNode` for **target**, and **properties**
...
from aoa.action_machine.graph.core.validation import require_non_empty_str
...
class BaseGraphEdge(ABC):
    ...
    CONTRACT: Concrete subclasses expose ``edge_relationship`` as their fixed :class:`~aoa.action_machine.graph.core.edge_relationship.EdgeRelationship`. ``target_node_id`` is a non-empty interchange id (``str``; typically matching another node's ``node_id``). ``target_node`` may be ``None`` until wired by a coordinator.
    Concrete subclasses implement :meth:`to_dict` with ``source_id`` for JSON export (edge row keys
    ...
    def to_dict(self, *, source_id: str) -> dict[str, Any]:
        """Return a JSON-serializable edge row for export (see :meth:`~aoa.action_machine.graph.core.node_graph_coordinator.NodeGraphCoordinator.to_json`)."""
