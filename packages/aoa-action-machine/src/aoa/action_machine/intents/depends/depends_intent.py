# packages/aoa-action-machine/src/aoa/action_machine/intents/depends/depends_intent.py
"""
...
2. **Type bound**: generic parameter ``T`` restricts which classes are
   ...
   - ``DependsIntent[DependsEligible]`` — only types that inherit
     :class:`~aoa.action_machine.intents.depends.depends_eligible.DependsEligible`
