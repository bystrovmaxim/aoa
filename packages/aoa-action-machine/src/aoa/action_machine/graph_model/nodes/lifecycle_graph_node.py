# packages/aoa-action-machine/src/aoa/action_machine/graph_model/nodes/lifecycle_graph_node.py
"""
...
Materializes a frozen :class:`~aoa.graph.base_graph_node.BaseGraphNode` for one
...
companion :class:`~aoa.action_machine.graph_model.nodes.state_graph_node.StateGraphNode` rows.
The parent lifecycle row keeps canonical companions in :attr:`~LifeCycleGraphNode.states`
(each a :class:`~aoa.action_machine.graph_model.nodes.state_graph_node.StateGraphNode`);
...
from aoa.action_machine.graph_model.edges.lifecycle_state_graph_edge import LifeCycleStateGraphEdge
...
class LifeCycleGraphPayload:
    """Frozen payload for :attr:`~aoa.graph.base_graph_node.BaseGraphNode.node_obj` on a lifecycle interchange row."""
...
class LifeCycleGraphNode(BaseGraphNode[LifeCycleGraphPayload]):
    ...
    INVARIANTS: Frozen interchange row; :attr:`states` list is filled once via :meth:`~aoa.action_machine.graph_model.edges.lifecycle_state_graph_edge.LifeCycleStateGraphEdge.get_state_edges` (**do not mutate** ``states`` nor its elements externally).
