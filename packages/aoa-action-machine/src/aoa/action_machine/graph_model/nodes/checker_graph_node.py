# packages/aoa-action-machine/src/aoa/action_machine/graph_model/nodes/checker_graph_node.py
"""
...
Materializes a frozen :class:`~aoa.graph.base_graph_node.BaseGraphNode` for a single checker
...
═══════════════════════════════════════════════════════════════════════════════
    ...
    Interchange graph: hosts register built ``CheckerGraphNode`` instances via ``companion_nodes`` on :class:`RegularAspectGraphNode`; inspectors flatten them into ``get_graph_nodes()`` (see :class:`~aoa.graph.base_graph_node.BaseGraphNode`).
"""
...
class CheckerGraphPayload:
    """Frozen payload for :attr:`~aoa.graph.base_graph_node.BaseGraphNode.node_obj` on a checker interchange row.

    :attr:`properties` merges the constructor ``properties`` checker-kwargs dict (if any) with ``"TypeChecker"`` (short kind for ``Field*Checker``) and ``"required"`` (same keys as on :class:`~aoa.graph.base_graph_node.BaseGraphNode`).
    """
