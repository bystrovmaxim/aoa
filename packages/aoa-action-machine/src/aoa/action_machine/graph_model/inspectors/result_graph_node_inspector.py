# packages/aoa-action-machine/src/aoa/action_machine/graph_model/inspectors/result_graph_node_inspector.py
"""
...
subtype that participates in interchange plus that node's :class:`~aoa.graph.base_graph_node.BaseGraphNode.companion_nodes` (``FieldGraphNode`` rows); opt out with :class:`~aoa.graph.exclude_graph_model.exclude_graph_model`.
...
═══════════════════════════════════════════════════════════════════════════════
    ...
    BaseResult  (axis root — skipped when decorated with ``exclude_graph_model``)
              │
              v
    each loaded strict subclass ``cls``  ->  ``ResultGraphNode(cls)`` plus :attr:`~aoa.graph.base_graph_node.BaseGraphNode.companion_nodes` (field rows) in the flat list when ``issubclass(cls, BaseResult)``
"""
