# packages/aoa-action-machine/src/aoa/action_machine/graph_model/inspectors/params_graph_node_inspector.py
"""
...
═══════════════════════════════════════════════════════════════════════════════
    ...
    BaseParams  (axis root — skipped when decorated with ``exclude_graph_model``)
              │
              v
    each loaded strict subclass ``cls``  ->  ``ParamsGraphNode(cls)`` plus :attr:`~aoa.graph.base_graph_node.BaseGraphNode.companion_nodes` (field rows) in the flat list when ``issubclass(cls, BaseParams)``
"""
