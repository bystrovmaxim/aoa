...
- **`LifecycleController` — canonical name for the lifecycle primitive.** `Lifecycle` is renamed to `LifecycleController` to align with the AOA primitive-language convention that all `BaseController` subclasses carry the `-Controller` suffix. The previous name is preserved as a module-level alias (`Lifecycle = LifecycleController`) for full backward compatibility — no import changes are required in existing code. `LifecycleController` is exported from `aoa.action_machine.domain` alongside `Lifecycle`. ([#83](https://github.com/bystrovmaxim/aoa/issues/83))
- **`BaseController` and `BaseCoordinator` marker base classes introduced.** Two empty base classes formalise the AOA primitive language. `BaseController` (`runtime/base_controller.py`) is the root for primitives that govern execution flow and state machines; `BaseCoordinator` (`runtime/base_coordinator.py`) is the root for primitives that extend machine behaviour. `Lifecycle` now inherits `BaseController`. `AuthCoordinator`, `NoAuthCoordinator`, `CacheCoordinator`, `SagaCoordinator`, `NodeGraphCoordinator`, `PluginCoordinator`, and `LogCoordinator` now inherit `BaseCoordinator`. Neither class carries logic — they serve as typed markers enabling `isinstance` checks and architectural documentation. `DebugNodeGraphCoordinator` inherits `BaseCoordinator` transitively through `NodeGraphCoordinator`. ([#83](https://github.com/bystrovmaxim/aoa/issues/83))
