...
- **`BaseAction.access_decide` — default object-level access hook.** New optional method, alongside `cache_key`/`read_cache`/`on_cache_write`: `async def access_decide(self, params, context, box, connections) -> bool`, defaulting to `True` (no restriction beyond roles/guard). Step 3 of the access-control-cascade plan; the runtime does not call it yet. ([#65](https://github.com/bystrovmaxim/aoa/issues/65))
- **`grant(role, when=...)` and `guard=` on `@check_roles`.** New `aoa.action_machine.intents.check_roles.grant`: `grant(role, when=None)` pairs a role with its own optional per-role condition. `grant(...)`/`guard=` are opt-in, not a legacy form to migrate away from — a bare role (or a list of roles, or `GuestRole`/`AnyRole`) stays the plain shorthand for "this role, no extra condition"; `@check_roles` accepts bare roles and `grant(...)` instances mixed freely as separate positional arguments, plus a new `guard=` keyword shared by every grant. Both flow through the interchange graph like every other `@check_roles` fact, not through a side-channel: `RoleGraphEdge` now carries one edge per grant with `when` in `edge.properties["when"]`, and `ActionGraphNode.properties["guard"]` carries the guard — both runtime-only, never exported by `to_dict()` (same treatment as `DependsGraphEdge`'s `factory`). `async def` in `when=`/`guard=` raises `AccessConditionAsyncError` at class definition time. Step 4 of the access-control-cascade plan; `RoleChecker` does not evaluate `when=`/`guard=` yet. ([#65](https://github.com/bystrovmaxim/aoa/issues/65))
- **`RoleChecker` evaluates `grant.when=` and `guard=` — level 2 of the access-control cascade.** `RoleChecker.check(context, action_node, params=None)` — new optional `params`, defaulting to `None` so existing two-argument call sites keep working unchanged. After a role matches, its grant's `when=` (read from `edge.properties["when"]`) is evaluated against `context.user`; grants are tried in declaration order with `any()` semantics — a matched role whose `when=` rejects does not stop the search, a later grant can still win. `grant(GuestRole, when=...)`/`grant(AnyRole, when=...)` are honored too — the sentinels' own bypass of role matching does not exempt their grant's `when=`. Once a grant wins, the action's `guard=` (`action_node.properties["guard"]`) is evaluated once against `context.user`/`params`. `AuthorizationError` gained an optional `level: int | None` (keyword-only, defaults to `None`): `1` when no role matched at all, `2` when a role matched but `when=`/`guard=` rejected the request. Step 5 of the access-control-cascade plan. ([#65](https://github.com/bystrovmaxim/aoa/issues/65))
- **`access_decide` (level 3) now runs on every `machine.run(...)` call.** New `ActionProductMachine._enforce_access_decide(action_instance, params, context, box, connections)`, called from `_run_internal` after `ToolsBox`/connections are built (both needed by `access_decide`'s signature from step 3) — `False` raises `AuthorizationError(level=3)`. Actions that never override `access_decide` (default `True`) are unaffected. Deliberately positioned *after* `emit_global_start`, not before: a level-3 denial must still be recorded as an attempted run, not vanish from telemetry — `RoleChecker.check` (levels 1-2) keeps its existing position, before any plugin lifecycle call, unchanged. Step 6 of the access-control-cascade plan. Precise start/finish timing for `access_decide` and cache work specifically is tracked as follow-up (see the ADR). ([#65](https://github.com/bystrovmaxim/aoa/issues/65))

### Fixed

- **Circular import between `BaseAction` and `Context`.** `model/base_action.py` imported `Context` at module level for `access_decide`'s type hint (step 3); `model/__init__.py` eagerly imports `base_action`, so any consumer that imports `context.context` before touching `model` (`aoa-otel`, `aoa-fastapi-adapter`, `aoa-mcp-adapter`) hit `ImportError: cannot import name 'Context' from partially initialized module`. `Context` now lives under `TYPE_CHECKING`, same treatment as `ToolsBox` (which has a directly confirmed cycle via `runtime.tools_box` importing `BaseAction`) — both are type-annotation-only, safe under `from __future__ import annotations`. ([#65](https://github.com/bystrovmaxim/aoa/issues/65))
