...
- **`RoleChecker`'s `FORBIDDEN_ROLE` denial reuses one frozen instance instead of rebuilding it per request.** No role matched at all (level 1) is the one denial `RoleChecker` decides on its own, always the same fixed reason — but the two sites that build it (`_check_sentinel`, `_denial_error`) each constructed a fresh `FailSecurityVerdict("FORBIDDEN_ROLE")`, re-running pydantic validation on an identical, frozen value, on this package's hottest path (every role-gated request that has no matching role at all) — the same class of waste `aoa-fastapi-adapter`'s `permissions.py` had already been fixed for (`_UNKNOWN_ENDPOINT_VERDICT`/`_UNAUTHORIZED_VERDICT`), just not applied here by analogy (baseverdict-audit finding 8, third document). New module-level `_FORBIDDEN_ROLE_VERDICT` constant, built once at import time; both sites now reference it. `FailSecurityVerdict`'s own import moves from a per-call deferred import (`TYPE_CHECKING` + local import, to dodge a transitive import cycle) to a plain top-level one — empirically confirmed clean across every import order and all four packages' test suites, so the cycle this file's own comment warned about no longer applies here.
