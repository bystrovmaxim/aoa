...
- **`reason=` is now a mandatory companion to `when=` on `grant()` and to `guard=` on `check_roles()`.** `grant(role, when=..., reason=...)`: give `when=`, and `reason=` is now required alongside it, or `grant()` raises `ValueError` at declaration. Same rule for `check_roles(..., guard=..., reason=...)`. `when=`/`guard=` themselves don't change (still plain boolean callables) — only their companion is now mandatory; several denial reasons for one role are still just several `grant()` calls, each with its own `when=`/`reason=` pair. `reason` travels the same path as the condition it belongs to, all the way to where `RoleChecker` decides: `RoleGraphEdge.properties["when_reason"]` next to `"when"` (one per grant), `ActionGraphNode.properties["guard_reason"]` next to `"guard"` (one per action) — `CheckRolesIntentResolver.resolve_guard_reason()` reads the latter back. A role that never matched at all still reports the framework-fixed `"FORBIDDEN_ROLE"`, never a developer-declared string.
- **`AccessVerdict`/`ResolveItemKind`/`ResolveItemResult` are replaced by `BaseVerdict` — a class hierarchy, one class per outcome, instead of one flat type plus an enum flag.** New `aoa.action_machine.intents.access_control.BaseVerdict` (abstract; cannot be instantiated directly) has three concrete subclasses: `AllowedVerdict` (the one way to say "yes" — no `reason` field at all, not even an empty one), `FailSecurityVerdict` (a real access-control denial; `reason: str` is mandatory and non-empty, `Field(min_length=1)`), and `FailErrorVerdict` (the check itself could not be answered — a crash, an unreachable connection, an operation with no registered route — never a denial, must never be cached as one; same mandatory non-empty `reason`). `kind` is not a free field a caller can set to a mismatched value: `BaseVerdict.__init__` fills it in from `type(self).__name__` when omitted and rejects an explicit `kind=` that disagrees with the class being constructed (see the `### Fixed` entry on why it is a stored field rather than a `@computed_field`). The closed set of possible `kind` values is exactly the set of `BaseVerdict` subclasses that exist, not a central `StrEnum` to keep in sync (`ResolveItemKind` is gone). Which `kind`/`reason` combinations are legal is no longer something a validator checks at runtime — it isn't expressible in the first place: `FailSecurityVerdict("")` is a pydantic `ValidationError` at construction, and `AllowedVerdict(reason=...)` doesn't type-check, because that class simply has no such field. **`access_decide(self, params, context, box, connections) -> FailSecurityVerdict | AllowedVerdict`** (was `-> bool`): return `AllowedVerdict()` to allow (the unchanged default when an action doesn't override it) or `FailSecurityVerdict(reason)` to deny with a specific, developer-declared reason — this is the cascade's own denial-reason mechanism for level 3, same as `grant(when=..., reason=...)`/`check_roles(guard=..., reason=...)` already are for levels 1/2. `reason=` on `grant()`/`check_roles()` is now a `FailSecurityVerdict` instance, not a bare string — `grant(role, when=..., reason=FailSecurityVerdict("..."))` — and defaults to a framework-owned `FailSecurityVerdict("FORBIDDEN_GRANT")`/`FailSecurityVerdict("FORBIDDEN_GUARD")` when the condition is given without an explicit `reason=`; `reason=` given without its condition is still a hard `ValueError` at declaration, unchanged. `AuthorizationError` carries the denial as `verdict: FailSecurityVerdict | None` (was a bare `reason: str | None`); its `.reason` property still reads `verdict.reason` for callers that only want the string. `machine.check_access_decide()` returns `BaseVerdict | list[BaseVerdict]` (was `AccessVerdict | list[AccessVerdict]`): a role/guard denial (`AuthorizationError` from `RoleChecker`) reports `exc.verdict`; any other exception anywhere in the check path (a bug in `access_decide`, an unreachable connection) becomes `FailErrorVerdict("EVALUATION_FAILED")` — never a denial, never cached as one, and never carrying the exception's own type or message (see the `EVALUATION_FAILED` entry under `### Fixed` for why the reason is fixed rather than descriptive). **This removes the empty-`reason`-string leak this design had been hardening against since fix-audit finding 1** (`grant(when=..., reason="")` slipping past an `is None` check and later reaching a caller as blank text): the earlier mitigations — `AuthorizationError.__init__` rejecting an empty `message`, `kind_matches_reason()` plus a `model_validator` enforcing `kind == SUCCESS ⟺ reason == ""` — are gone, not because the bug came back, but because `reason: str = Field(min_length=1)` makes the state they used to guard against impossible to construct at all. **`ResolveItemResult` — the shared wire shape this hierarchy briefly lived under (as `AccessVerdict`, a `ResolveItemResult` subclass adding `action`/`action_name` fields) — no longer exists in any form.** `BaseVerdict` itself, exported from `aoa.action_machine.intents.access_control`, is both the in-process access-check answer and the wire type `aoa-fastapi-adapter`'s resolver returns directly (see that package's own CHANGELOG) — no separate wire-projection step, and no `action`/`action_name` field on any subclass: nothing reads it back today, so it wasn't kept. Every existing `grant(when=...)`/`check_roles(guard=...)` call site, and every reader of `AccessVerdict.allowed`/`.level`/`.kind`/`.action_name` or `ResolveItemKind`, must be updated. ([#130](https://github.com/bystrovmaxim/aoa/issues/130))
...
- **`FORBIDDEN_OBJECT` — one shared, reusable `FailSecurityVerdict` for object-level generic deny.** New module-level constant in `access_verdict.py` (`aoa.action_machine.intents.access_control`, `FailSecurityVerdict("FORBIDDEN_OBJECT")`), exported alongside `AllowedVerdict`/`BaseVerdict`/`FailErrorVerdict`/`FailSecurityVerdict`. An `access_decide()` that can tell "no such object" apart from "object belongs to someone else" by its *reason text* is an oracle: a caller probing IDs learns which objects exist for other users, even though every response is a denial. Both cases are meant to return this exact same instance — `CancelOrderAction` (`aoa-demo`) is the reference usage, checking existence and ownership together in one branch on purpose (see the constant's own comment for why a separate existence-then-ownership check is the wrong shape even when both branches currently return the same verdict: two branches drift apart when edited, and they take measurably different work, which leaks the same fact through timing). A more specific reason (e.g. "order is already cancelled") stays safe to return once ownership is already confirmed — a non-owner probing the same object ID still gets plain `FORBIDDEN_OBJECT`, never the specific text. ([#159](https://github.com/bystrovmaxim/aoa/issues/159))
- **Access-control cascade: `grant`/`guard=` on `@check_roles`, `access_decide`, and `machine.check_access_decide`.** Authorization now has three levels, each able to deny before the next runs. Level 1 (role) gains `grant(role, when=...)` — a role paired with its own optional condition on the caller, tried in declaration order with `any()` semantics; a bare role stays the permanent shorthand for "no condition", freely mixable with `grant(...)` instances. Level 2 is a new `guard=` keyword on `@check_roles` — one condition shared by every grant, evaluated after a grant wins, seeing both the caller and the call's params (`guard(user, params)`), unlike `grant.when=(user)`. Level 3 is `BaseAction.access_decide(self, params, context, box, connections) -> FailSecurityVerdict | AllowedVerdict` — an object-level fact check (e.g. "is this the caller's own order"), defaulting to `AllowedVerdict()`; it runs on every `machine.run()` call, after the same role/guard gate. `grant.when=`/`guard=` must be synchronous and return `bool`; an `async def` raises `AccessConditionAsyncError` at class definition, not at runtime. Denial at any of the three levels is `AuthorizationError` with a `level` attribute (`1`/`2`/`3`) identifying which gate rejected the call; like existing role denials, it flies past `@on_error`/the saga untouched, since all three gates run before the aspect pipeline starts. Finally, `machine.check_access_decide(context, action, params=None, *, connections=None) -> BaseVerdict` (and its list-of-`(action, params)` overload, returning `list[BaseVerdict]`) asks "would this be allowed?" without running the operation — no pipeline, cache, or plugin events; a denial becomes `FailSecurityVerdict(reason)` instead of an exception (see the `BaseVerdict` hierarchy entry above). The list form is the primitive (one throwaway `ToolsBox` per item, one item's failure never affecting the others); the single-action form recurses into it. ([#65](https://github.com/bystrovmaxim/aoa/issues/65))

### Fixed

- **`grant()`/`check_roles()` — one `require_reason_alongside()` helper replaces two word-for-word-identical `reason=` validations.** Both rejected `reason=` given without its condition, and both defaulted the condition-without-`reason=` case to a framework-owned `FailSecurityVerdict`, differing only in the condition's name (`when`/`guard`) and the default string (`"FORBIDDEN_GRANT"`/`"FORBIDDEN_GUARD"`) — the same check, fixed once for each site independently instead of factored out once (fix-audit finding 15, second document). New `intents/check_roles/reason_validation.py`; both `grant.py` and `check_roles_decorator.py` call it instead of repeating the check.
- **`AuthorizationError("")` with no `verdict=` is rejected again.** A guard closing exactly this gap shipped once (fix-audit finding 1, second document: an empty `AuthorizationError` could reach `resolve_verdicts()`'s `asyncio.gather(...)` — no `return_exceptions=True` — as an uncaught `pydantic.ValidationError` from constructing `FailSecurityVerdict("")` inside `check_access_decide`'s own `except AuthorizationError` handler, crashing an entire batch over one bad item) — then silently disappeared as a side effect of the `BaseVerdict` refactor (`6b4dd958`) renaming the parameter the guard was written against (`reason: str` → `verdict: FailSecurityVerdict | None`). The guard is restored against the new signature: `AuthorizationError(message, *, verdict=...)` now raises `ValueError` when `message` and `verdict` are both empty. No existing call site constructs one that way today, so nothing else changes (fix-audit finding 1, third document).
- **`access_decide()` — the real execution path (`machine.run()`) and the check-only path (`machine.check_access_decide()`) both now fail closed on an unexpected return value, not just an explicit denial.** Level 3 used to gate on `isinstance(verdict, FailSecurityVerdict)` — block only what is *provably* a denial, silently treat everything else (`None` from a forgotten `return` in someone's override, a stray `bool`, a `FailErrorVerdict` returned by mistake) as an allow. Fail-open, backwards from every other gate in the cascade (baseverdict-audit finding 2, third document — the most severe of the three documents: a bug in an action's own `access_decide` override, not an attacker, was enough to silently open access). New `_validated_access_decide_verdict()` rejects anything that isn't `AllowedVerdict`/`FailSecurityVerdict` with a `TypeError` — a bug in the override, not an ambiguous or partial answer, so it is never folded into "allowed" or "denied". Each caller's own existing exception handling decides what that means: `_enforce_access_decide` lets it propagate and abort the real call (fail closed); `check_access_decide`'s list form already turns any non-`AuthorizationError` exception into an isolated `FailErrorVerdict` for that one item, so this reuses that path rather than adding new branching. **The error message names only `type(verdict).__name__`, never `verdict` itself** — a broken override could return anything, including a domain object carrying sensitive data, and (unlike the check-only path, which keeps nothing of the exception at all -- just the fixed `EVALUATION_FAILED`) the real `machine.run()` path lets this exception propagate uncaught, so its text can reach server logs or an HTTP 500 body.
- **`grant(when=..., reason=...)`/`check_roles(guard=..., reason=...)` — `reason=` must actually be a `FailSecurityVerdict`, checked at declaration.** `require_reason_alongside()` only checked `reason is not None`, not its type — `reason` was a plain `str` before the `BaseVerdict` redesign, and the parameter's `FailSecurityVerdict | None` annotation is not enforced by Python at runtime, so an ordinary string (migrated code, or simply not run through mypy) passed silently through construction. It then reached `AuthorizationError.verdict` and crashed the first time anything read `.verdict.reason` off it — `AttributeError: 'str' object has no attribute 'reason'` — in production, at the exact moment a real denial needs to explain itself (baseverdict-audit finding 3, third document). `require_reason_alongside()` now rejects a non-`None` `reason` that isn't a `FailSecurityVerdict` instance (subclasses included) with a `TypeError`, before any other check runs.
- **`BaseVerdict.kind` is a real, stored field again — filled in and validated by `__init__`, not a `@computed_field`.** The computed-field design gave an unconditional guarantee (`kind` is always exactly `type(self).__name__`, no matter how an instance is built) but had two real, reproduced costs: `kind` silently vanished from `BaseSchema`'s dict-like interface (`.keys()`/`.items()`/`in` only look at declared `model_fields`, not `model_computed_fields`, so `list(AllowedVerdict().items())` was `[]` even though `verdict["kind"]` worked fine — baseverdict-audit finding 6, third document), and a verdict's own `model_dump()` output could never be fed back into `model_validate()` on the same class (`kind` is present in the dump but rejected as an unknown field on the way back in, `extra="forbid"` — finding 4). `kind: str = Field(default="")` is now declared normally (the default is never actually used — `BaseVerdict.__init__` always supplies a real value, and the `Field(default=...)` exists only so mypy's pydantic plugin treats `kind` as optional for subclasses like `AllowedVerdict` that don't override `__init__`); `__init__` fills it in from `type(self).__name__` when omitted, and raises `ValueError` if a caller passes a `kind=` that disagrees with the class being constructed — same guarantee, enforced explicitly instead of inherited implicitly. Both single-class round-tripping (`FailSecurityVerdict.model_validate(v.model_dump())`) and the dict interface now work correctly. **Trade-off, made deliberately:** `model_construct()`/`model_copy(update=...)` (pydantic's validation-skipping escape hatches, already documented as a caveat for `reason` — see the fix-audit-finding-5 entry above) can now also produce a verdict whose `kind` disagrees with its real Python class, e.g. a `FailSecurityVerdict` claiming `kind="AllowedVerdict"` — something the old computed-field design made structurally impossible even through those bypasses. Nothing in this codebase calls either method on a `BaseVerdict` today. The container-level round-trip (a whole `ResolveResponse.results` list deserializing back into the correct mix of concrete subclasses) still does not work without a discriminated union keyed on `kind` — a separate, larger design decision, deliberately not made here since it would reintroduce a central registry of subclasses, the exact thing this class hierarchy was designed to avoid.
- **`RoleChecker`'s `FORBIDDEN_ROLE` denial reuses one frozen instance instead of rebuilding it per request.** No role matched at all (level 1) is the one denial `RoleChecker` decides on its own, always the same fixed reason — but the two sites that build it (`_check_sentinel`, `_denial_error`) each constructed a fresh `FailSecurityVerdict("FORBIDDEN_ROLE")`, re-running pydantic validation on an identical, frozen value, on this package's hottest path (every role-gated request that has no matching role at all) — the same class of waste `aoa-fastapi-adapter`'s `permissions.py` had already been fixed for (`_UNKNOWN_ENDPOINT_VERDICT`/`_UNAUTHORIZED_VERDICT`), just not applied here by analogy (baseverdict-audit finding 8, third document). New module-level `_FORBIDDEN_ROLE_VERDICT` constant, built once at import time; both sites now reference it. `FailSecurityVerdict`'s own import moves from a per-call deferred import (`TYPE_CHECKING` + local import, to dodge a transitive import cycle) to a plain top-level one — empirically verified against several import orders and all four packages' test suites for *this file's* current position in the import graph (baseverdict-audit finding 3, fourth document: not a claim about the technique in general — the identical move applied to the neighboring `grant.py`, which carries the same guarding comment, reproducibly raises `ImportError` from a real cycle there).
- **`Grant`, constructed directly instead of through `grant()`, now validates `reason=` too.** `grant()`'s own `reason=`/`when=` pairing-and-type validation (fix-audit finding 3, third document) lived only in the factory function's body — `Grant` itself, a public, independently-importable `@dataclass(frozen=True)`, had no validation of its own, so constructing it directly (`Grant(role=AdminRole, when=cond, reason="not eligible")`) bypassed the check entirely and reached the exact same crash the earlier fix closed, just through a different door (baseverdict-audit finding 1, fourth document). `Grant.__post_init__` now calls the same `require_reason_alongside()` `grant()` already used, so the guarantee holds regardless of which entry point constructed the instance; `grant()` itself is now a thin wrapper that only validates `role`.
- **`AuthorizationError.__init__` now validates `verdict=`'s type, not only its absence.** The empty-message-and-empty-verdict guard (fix-audit finding 1, second document, restored by baseverdict-audit finding 1, third document) and `require_reason_alongside()`'s own `reason=` type check (baseverdict-audit finding 3, third document) both closed the identical class of gap elsewhere — but `AuthorizationError.__init__` itself, the very function that received the emptiness guard two commits earlier in the same range, never got the matching type check: `verdict=` accepted a plain string, or a real `BaseVerdict` subclass of the wrong kind (`AllowedVerdict` instead of a denial), with no error until the first read of `.reason` — which happens in the shared HTTP 403 handler, on every real denial, with no `try/except` around it (baseverdict-audit finding 2, fourth document). `__init__` now raises `TypeError` when `verdict` is given but is not a `FailSecurityVerdict` instance, alongside the existing emptiness guard.
- **`check_access_decide`'s per-item crash handler no longer leaks the crashing exception's class name as the denial reason.** Refines the `BaseVerdict` hierarchy entry above, in this same unreleased block, rather than a shipped release. A bug in an `access_decide()` override, or an unreachable connection, already became an isolated `FailErrorVerdict` for that one item without aborting the rest of the batch — but the reason was `type(exc).__name__` (`ValueError`, `ConnectionError`, ...), a second, independent way for the response to reveal *why* a check failed, the same class of oracle risk `FORBIDDEN_OBJECT` closes for denials. `action_product_machine.py`'s `check_access_decide` now always reports the fixed reason `"EVALUATION_FAILED"` regardless of what actually crashed; the crashing exception's own type and message are not recorded anywhere in this path today — the check path deliberately runs without plugin events, so there is no AOA-principled channel to emit them through, and an ad-hoc log call is not one. Restoring that server-side visibility the proper way is tracked separately in [#160](https://github.com/bystrovmaxim/aoa/issues/160). `FailErrorVerdict`'s own docstring updated to match. ([#159](https://github.com/bystrovmaxim/aoa/issues/159))

### Documentation

- **`BaseVerdict`/`FailSecurityVerdict` docstrings no longer claim an empty-`reason` denial is impossible, full stop.** The guarantee (`Field(min_length=1)`, `frozen=True`) only holds for the normal constructor — pydantic's own `model_construct()`/`model_copy(update=...)` are documented escape hatches that skip validation entirely and can still produce e.g. `FailSecurityVerdict(reason="")` (confirmed empirically; same already-established caveat this codebase documents elsewhere for `model_construct()`, in `json_schema_value.py`). Nothing calls either method on a `BaseVerdict` today, so this is a documentation fix only, not a behavior change (baseverdict-audit finding 5, third document).
