...
OCEL export (``StoreDomain``): lifecycle actions ``PublishOrder*OcelAction`` on ``CustomerAccountEntity`` / ``SalesOrderEntity``; batch via ``run_store_ocel_trace_batch`` → ``archive/logs/ocel.json`` (integration test). Single-trace smoke: ``test_store_ocel_export.py``.
