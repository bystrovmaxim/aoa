# packages/aoa-demo/src/aoa/demo/fastapi_mcp_services/app_fastapi_service.py
"""
...
═══════════════════════════════════════════════════════════════════════════════
    ...
    uvicorn aoa.demo.fastapi_mcp_services.app_fastapi_service:app --reload

    Same module starts uvicorn when run as a file::

        python …/packages/aoa-demo/src/aoa/demo/fastapi_mcp_services/app_fastapi_service.py
        python …/app_fastapi_service.py --host 0.0.0.0 --port 8080

    …/ pytest ``pythonpath`` (see ``pyproject.toml``) includes each package ``…/src`` so ``aoa.*`` imports resolve.
...
from aoa.demo.fastapi_mcp_services.actions import CreateOrderAction, GetOrderAction, PingAction
from aoa.demo.fastapi_mcp_services.infrastructure import auth, machine
from aoa.demo.model.actions import GetExampleModelGraphJsonAction
